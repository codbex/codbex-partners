angular.module('page', ['blimpKit', 'platformView', 'EntityService'])
	...
	.controller('PageController', ($scope, $http, EntityService, Extensions, ButtonStates) => {
		...
		Dialogs.addMessageListener({ topic: 'codbex-partners.Manufacturers.Manufacturer.entitySearch', handler: () => {
			resetPagination();
		...
		$scope.loadPage = (pageNumber, filter) => {
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					...
					Dialogs.showAlert({
						title: 'Manufacturer',
						message: `Unable to list/filter Manufacturer: '${message}'`,
				...
				Dialogs.showAlert({
					title: 'Manufacturer',
					message: `Unable to count Manufacturer: '${message}'`,
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
		...
		$scope.deleteEntity = () => {
			...
			Dialogs.showDialog({
				...
				message: `Are you sure you want to delete Manufacturer? This action cannot be undone.`,
				buttons: [{
					id: 'delete-btn-yes',
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: 'Manufacturer',
							message: `Unable to delete Manufacturer: '${message}'`,
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
