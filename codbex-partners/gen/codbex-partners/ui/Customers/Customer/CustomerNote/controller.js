angular.module('page', ['blimpKit', 'platformView', 'EntityService'])
	...
	.controller('PageController', ($scope, EntityService, Extensions, ButtonStates) => {
		...
		$scope.loadPage = (pageNumber, filter) => {
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					...
					Dialogs.showAlert({
						title: 'CustomerNote',
						message: `Unable to list/filter CustomerNote: '${message}'`,
				...
				Dialogs.showAlert({
					title: 'CustomerNote',
					message: `Unable to count CustomerNote: '${message}'`,
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
		...
		$scope.deleteEntity = (entity) => {
			let id = entity.Id;
			Dialogs.showDialog({
				title: 'Delete CustomerNote?',
				message: `Are you sure you want to delete CustomerNote? This action cannot be undone.`,
				buttons: [{
					id: 'delete-btn-yes',
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: 'CustomerNote',
							message: `Unable to delete CustomerNote: '${message}'`,
							type: AlertTypes.Error,
						});
						console.error('EntityService:', error);
					});
