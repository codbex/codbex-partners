angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-partners:codbex-partners-model.defaults.deleteTitle', { name: '$t(codbex-partners:codbex-partners-model.t.CUSTOMERNOTE)' });
			translated.deleteConfirm = LocaleService.t('codbex-partners:codbex-partners-model.messages.deleteConfirm', { name: '$t(codbex-partners:codbex-partners-model.t.CUSTOMERNOTE)' });
		});
		//-----------------Custom Actions-------------------//
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('codbex-partners:codbex-partners-model.messages.error.unableToLF', { name: '$t(codbex-partners:codbex-partners-model.t.CUSTOMERNOTE)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-partners:codbex-partners-model.t.CUSTOMERNOTE'),
					message: LocaleService.t('codbex-partners:codbex-partners-model.messages.error.unableToCount', { name: '$t(codbex-partners:codbex-partners-model.t.CUSTOMERNOTE)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-partners:codbex-partners-model.t.CUSTOMERNOTE'),
							message: LocaleService.t('codbex-partners:codbex-partners-model.messages.error.unableToDelete', { name: '$t(codbex-partners:codbex-partners-model.t.CUSTOMERNOTE)', message: message }),
							type: AlertTypes.Error,
						});
						console.error('EntityService:', error);
					});
