...
const dao = require("codbex-partners/gen/dao/settings/PartnerType");
const http = require("codbex-partners/gen/api/utils/http");
...
rs.service()
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("PartnerType not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/js/codbex-partners/gen/api/PartnerType.js/" + entity.Id);
