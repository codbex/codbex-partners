angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-partners-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "Suppliers" && e.view === "Supplier" && (e.type === "page" || e.type === undefined));
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("Supplier", `Unable to list/filter Supplier: '${response.message}'`);
						return;
					}
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("Supplier-filter", {
