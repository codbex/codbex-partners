angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', function ($scope, $http, messageHub, entityApi) {
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("CustomerNote", `Unable to list/filter CustomerNote: '${response.message}'`);
						return;
					}
		...
		$scope.openDetails = function (entity) {
			$scope.selectedEntity = entity;
			messageHub.showDialogWindow("CustomerNote-details", {
				action: "select",
				entity: entity,
			});
		};
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("CustomerNote-filter", {
		...
		$scope.deleteEntity = function (entity) {
			...
			messageHub.showDialogAsync(
				'Delete CustomerNote?',
				`Are you sure you want to delete CustomerNote? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("CustomerNote", `Unable to delete CustomerNote: '${response.message}'`);
							return;
						}
						$scope.loadPage($scope.dataPage, $scope.filter);
